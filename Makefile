...
local: check
	make build-init
	$(eval OBS_WORKDIR := $(BUILD_TARGET)/$(SOURCEGUARDIAN_TARGET))
...
obs: check sourceguardian54 sourceguardian55 sourceguardian56

build-binary-target:
	make build-init
	$(eval OBS_WORKDIR := $(BUILD_TARGET)/$(SOURCEGUARDIAN_TARGET))
	cat macros/$(SOURCEGUARDIAN_TARGET) SPECS/scl-sourceguardian.spec > OBS/$(OBS_WORKDIR)/scl-sourceguardian.spec
...
sourceguardian54 sourceguardian55 sourceguardian56:
	SOURCEGUARDIAN_TARGET=$@ make build-binary-target
...
build-init: build-clean
	$(eval OBS_WORKDIR := $(BUILD_TARGET)/$(SOURCEGUARDIAN_TARGET))
	mkdir OBS
	osc branch $(OBS_PROJECT) $(OBS_PACKAGE) $(BUILD_TARGET) $(SOURCEGUARDIAN_TARGET) 2>/dev/null || exit 0
